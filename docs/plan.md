...
- [ ] **DrillPanel 改 2-tab 结构**（`src/components/drill/DrillPanel.tsx` 扩展）
  - tab strip：`Detail`（现 v0.4 内容） / `Conversation`（新增）
  - tab 状态在 store（`drillPanelTab: "detail" | "conversation"`），localStorage 持久化（key `loomscope:drillPanelTab`）
  - 两 tab **共享 selection state**：Conversation tab 点某条消息 → 左侧 canvas 焦点跟着移；canvas 选中节点 → Conversation tab 自动滚到对应消息并高亮
- [ ] **ConversationView 面板**（新增 `src/components/ConversationView.tsx`，移植 Agentloom `pathUtils.ts`）—— Conversation tab 内容
  - root → `selectedNodeId` 的线性链显示
  - **视觉规范**：参考 Claude App 对话视图——纵向滚动；user 消息右对齐 + 浅蓝底；assistant 消息左对齐 + 白底；MarkdownView 渲染（v0.4 现成）；每条消息附 token 数 + 时间戳 chip
...
- **Composer 入口位置**：v0.8 落地的 Conversation tab 底部加 input box（参考 Claude App，pinned bottom + 多行支持 + Cmd/Ctrl+Enter 提交）
- 提交逻辑：发 prompt 续接已存 session（**仅 leaf-continuation**——任意起点 fork 是 v∞.3）；新 turn 的 `parentUuid = 当前 focused 节点的末 assistant uuid`，但 v∞.2 强制 focused 节点 = leaf
...
- **Composer 复用 v∞.2 的输入框**：v∞.2 已经在 Conversation tab 底部装好 input box；v∞.3 的差异是**解除 leaf-only 限制**——focused 节点可以是任意 ChatNode（含 assistant、旁支 sibling），composer 自动用 focused 节点的末 assistant uuid 作为 parentUuid
...
- **额外动作**：Conversation tab 顶部加"导出为独立 session"按钮（CC `/branch` 等价，但任意起点）

#### 依赖

- v0.8 fork 浏览（前置）—— ConversationView / branchMemory / fork badge / merged ChatFlow / 2-tab DrillPanel / Conversation tab 必须先有
- v∞.2 composer + Agent SDK 接入（前置）—— write 能力本身 + Conversation tab 底部 input box
