...
### 256MB selection 切换性能 — 留 v0.9 polish
...
修法：让 selected 走 React Flow 内置 selection state（`Node.selected` field 由 RF 自己 track），不再 prop 注入。属于 v0.9 polish 范畴，跟 v0.4 panel 解耦。
...
- v0.6 compact 完整交互（`CompactDetail` 已留提示）
...
- **Selection 切换性能**（v0.4 暴露）：把 `selected` 字段从 `decoratedNodes = nodes.map(...)` props 注入路径迁出，让 React Flow 内置 selection state 自己 track，避免 1500-ChatNode 全图 reconcile（实测 458ms avg → 目标 < 100ms）
