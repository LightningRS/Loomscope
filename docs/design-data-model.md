...
### file-history-snapshot binding (v0.7 实测纠正)

> **v0.7 实测推翻了 v0.1 的"全 orphan + 时间窗启发"假设。**

v0.1 doc 说："2099 条 file-history-snapshot 全部 `parentUuid:null` + 没 promptId，没法 bucket，全进 orphans，v0.7 用时间窗反推。"

v0.7 实测（跨用户 3059 条 snapshot 全集）：
- **100% 有 `messageId`**（顶层 OR 嵌套在 `snapshot.messageId`）
- **99.97% 的 messageId 直接 lookup 到一条 user/assistant record**
- 67% 的 record 直接带 `promptId`；剩下 33% 是 assistant record，走 `resolvePromptId(parentUuid)` 一跳就解析出 promptId
...
3. `logicalParentUuid` 跨断点指向**当前段最后一个 turn_duration**——给 viewer 一条弱边，可视化上画虚线连过去 (v0.7 ship: dashed 浅灰反向弧)
4. **v0.7 parser pre-resolves**: `CompactNode.logicalParentChatNodeId = resolvePromptId(boundary.logicalParentUuid)` —— pre-compact 段尾巴的 ChatNode promptId. compact-original drill 路径用这个起点沿 parentChatNodeId 反向追溯（见下）

### compact ChatNode 的 inner workflow 实测发现 (v0.7)

> **CC 主 jsonl 里 compact 的"压缩动作 LLM 调用"完全不可见**——新版本 (v2.1.104+) inline 处理后只把 summary 结果写盘，那次 LLM call 本身 0 痕迹；旧版本 (v2.1.94) 的"压缩 LLM 调用"在 `subagents/agent-acompact-<id>.jsonl` 里完整可见。

但 compact ChatNode 的 inner workflow **不是"压缩过程"**，是**用 summary 作为新起点续接的整段对话**——CC 的 promptId bucket 把"compact 触发后续接的 LLM/tool 全归到同一个 ChatNode 里"，直到下一个用户 prompt 才切到新 ChatNode。

实测 256MB session 131 个 compact ChatNode：
- 128 (97.7%) inner workflow 含 llm_call
- 123 (94%) 含 tool_call
- 0 (0%) 只有一个 CompactNode 没其它东西
- 总 inner: 12738 llm_call + 8369 tool_call + 845 attachment + 22 delegate

⇒ **v0.7 ChatNodeCard.CompactCard** 给 compact ChatNode 两个 drill 入口：
1. "进入工作流" → 看 inner workflow 的 post-compact 续接对话（128/131 有 llm_call 时显示）
2. "⤢ 展开 pre-compact" → drill 进 pre-compact 原段（compact-original DrillFrame，参见 plan.md v0.7）

### compact-original drill 范围 (v0.7)

drill 进 compact ChatNode → **合成 ChatFlow**，递归喂给 ChatFlowCanvas 渲染（复用 v0.6 redo sub-chatflow drill 框架）。范围算法（`computePreCompactRange`）：

1. 起点：`compactCn.compactMetadata.logicalParentChatNodeId`（pre-compact 段尾巴 ChatNode，由 parser pre-resolve）
2. 终点：沿 `parentChatNodeId` 反向追溯，直到：
   - session root（`parentChatNodeId === null`），OR
   - 上一个 compact ChatNode（不包含——它代表更早一次 compaction，本身已是 fold 标记）
3. 范围按时间正序返回；合成 ChatFlow 的头节点 `parentChatNodeId` rewrite 为 null（避免 layoutDag 画悬空 edge）
